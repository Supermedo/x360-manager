...
### Fixed
- Canary download now uses the current `xenia-canary/xenia-canary` releases (daily builds) instead of the stale `xenia-canary-releases` mirror

## [1.6.8] - 2026-09-16

### Fixed
- Xenia Canary download failing/crashing — GitHub renamed the Windows asset (`xenia_canary_windows.zip` → `xenia_canary_windows_.zip` / `.7z`); the app now resolves the real release asset and extracts both zip and 7z
- FPS overlay helper text clarifies that the counter is capture-based (often ~30) and many Xbox 360 games are native 30 FPS

### Changed
- Bundled `7zip-bin` so Canary `.7z` builds can be extracted without PowerShell
