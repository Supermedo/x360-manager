...
execSync(`npx electron-builder --win nsis -p never --config.directories.output=${path.basename(tempOut)}`, {
  cwd: root,
  stdio: 'inherit',
  env: { ...process.env }
});
...
for (const name of [exeName, `${exeName}.blockmap`]) {
...
const buildInfo = [
  `X360 Manager ${version}`,
  `Built: ${new Date().toISOString()}`,
  `Installer: ${exeName}`,
  'Includes: Xenia config fix, language override, game settings (v1.6.2)'
].join('\n');
